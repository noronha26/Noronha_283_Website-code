...
function SignUpBox({setCompStatus,compStatus}) {
  ...
  const handleGenrateOtp=()=>{
    axios.post(`${process.env.NEXT_PUBLIC_URL}/users/genrate-otp`,data)
  ...
  const handelSignup=()=>{
    axios.post(`${process.env.NEXT_PUBLIC_URL}/users/register-user`,data)
  ...
  return (
    ...
    <input  onChange={(e)=>{setdata({...data,otp:e.target.value})}}
      name="otp"
    ...
    {/* <div className="text-[14px] flex gap-5 font-medium">I shop for 
     ...
     {/* <div class="flex gap-2 pt-3">
         ...
         type="button" className="p-3.5 mt-4 w-full bg-black text-white font-semibold">Genrate OTP</button>
      <button
      onClick={handelSignup}
       type="button" className="p-3.5 mt-4 w-full bg-black text-white font-semibold">Sign Up</button>
     </div>
        </div>
       </div>
    </div>
