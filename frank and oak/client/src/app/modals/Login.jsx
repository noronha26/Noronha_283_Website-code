...
export default function Login({ loginStatus, setLoginStatus }) {
  ...
  const handleLoginUser = async (e) => {
    e.preventDefault();
    setLoading(true);
    const email = e.target.email.value; // 'email' is the name of the input
    const password = e.target.password.value; // 'password' is the name of the input
    ...
    try {
      ...
      const response = await axios.post(
        `${process.env.NEXT_PUBLIC_URL}/users/login-user`,
        { email, password }
      );

      console.log("Response received:", response.data);
      Cookies.set("user_290_283", JSON.stringify(response.data.data));
...
function LoginBox() {
  ...
  // useEffect(() => {
  //   // Ensuring router is only accessed client-side
  //   if (typeof window !== "undefined") {
  //     // Handle side effects related to router if needed
  //   }
  // }, []);
...
function SignUpBox({ setCompStatus, compStatus }) {
  ...
  const handleGenrateOtp = () => {
    axios
      .post(`${process.env.NEXT_PUBLIC_URL}/users/genrate-otp`, data)
  ...
  const handelSignup = () => {
    axios
      .post(`${process.env.NEXT_PUBLIC_URL}/users/register-user`, data)
      ...
      <input type="radio" name="radio" id="" /> All
          ...
          <div>
            ...
            <button
              ...
              className={`${
                !ifotp ? "hidden" : "block"
              } p-3.5 mt-4 w-full bg-black text-white font-semibold`}
            >
              Sign Up
            </button>
          </div>
