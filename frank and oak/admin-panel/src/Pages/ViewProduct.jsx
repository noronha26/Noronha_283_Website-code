...
const ViewProduct = () => {
  ...
  const handleFetchProduct=()=>{
    axios.get(`${process.env.REACT_APP_API_URL}admin-panel/product/read-product`)
    .then((response)=>{
  ...
  const handeleUpdateProducts=(e)=>{
    ...
    axios.put(`${process.env.REACT_APP_API_URL}admin-panel/product/updateProduct-status/${e.target.value}`,{status})
    .then((response)=>{
      console.log(response.data)
      Swal.fire({
        position: "top-end",
  ...
  const handleDeleteProduct=(id)=>{
    Swal.fire({
      ...
      if (result.isConfirmed) {
        axios.delete(`${process.env.REACT_APP_API_URL}admin-panel/product/delete-product/${id}`)
        .then((response)=>{
          console.log(response.data)
        ...
        <table className="w-full">
          ...
          <Tooltip id="status-tooltip" />
            ...
            products.map((product,index) => (
                ...
                <td>
                  <MdDelete onClick={()=>{handleDeleteProduct(product._id)}} className="my-[5px] text-red-500 cursor-pointer inline" />{" "}
                  |{" "}
                ...
                <td>
                 ...
                 data-tooltip-content={`Click to ${(product.status)?'Inactive':'Active'}`}
                  className={`p-4 rounded-sm border ${(product.status)?
                  'bg-green-500'
                  :
                  'bg-red-500'
                } text-white`}>
