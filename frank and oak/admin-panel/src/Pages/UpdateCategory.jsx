...
const UpdateCategory = () => {
  ...
  const fetchCategory=()=>{
    axios.get( `${process.env.REACT_APP_API_URL}admin-panel/parent-category/read-category/${_id}`)
    .then((response)=>{
  ...
  useEffect(()=>{fetchCategory();},[_id]);

  const handleUpdateCategory=(e)=>{
    e.preventDefault();
    axios.put( `${process.env.REACT_APP_API_URL}admin-panel/parent-category/update-category/${_id}`,e.target)
    .then((response)=>{
      console.log(response.data);
      let timerInterval;
      Swal.fire({
      ...
      <div className="w-[90%] mx-auto my-[20px]">
        <form method="post" onSubmit={handleUpdateCategory} >
          ...
          <div className="w-full my-[20px] ">
            <button className="bg-[#5351c9] rounded-md px-2 text-white h-[35px]">
              Update Category
            </button>
          </div>
        </form>
